...
protected:
    ...
    bool init(int argc, const char* argv[]) override
    {
        ...
        m_shadow_map.set_direction(/*m_sky_model.direction()*/ glm::vec3(1.0f));
        m_shadow_map.set_extents(12.0f);

        glEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS);

        m_position_transform = glm::translate(m_position_transform, glm::vec3(0.0f, 3.0f, 0.0f));
...
private:
    ...
    void particle_initialize()
    {
        ...
        glDispatchCompute((GLuint)ceil(float(MAX_PARTICLES) / float(LOCAL_SIZE)), 1, 1);
    ...
    void load_mesh()
    {
        m_playground = dw::Mesh::load(FileSystem::getPath("resources/objects/rock/rock.obj"));
    }
    ...
    {
        {
            ...
            m_particle_vs                = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_VERTEX_SHADER, "particle.vs"));
            m_particle_fs                = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_FRAGMENT_SHADER, "particle.fs"));
            m_particle_initialize_cs     = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_COMPUTE_SHADER, "particle_initialize.cs"));
            m_particle_update_kickoff_cs = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_COMPUTE_SHADER, "particle_kickoff.cs"));
            m_particle_emission_cs       = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_COMPUTE_SHADER, "particle_emission.cs"));
            m_particle_simulation_cs     = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_COMPUTE_SHADER, "particle_simulation.cs"));
            m_mesh_vs                    = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_VERTEX_SHADER, "mesh.vs"));
            m_mesh_fs                    = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_FRAGMENT_SHADER, "mesh.fs"));
            m_depth_fs                   = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_FRAGMENT_SHADER, "depth.fs"));
            m_depth_prepass_fs           = std::unique_ptr<dw::gl::Shader>(dw::gl::Shader::create_from_file(GL_FRAGMENT_SHADER, "depth_prepass.fs"));
    ...
    void update_color_over_time_texture()
    {
        ...
        for (uint32_t i = 0; i < GRADIENT_SAMPLES; i++)
        {
            ...
            samples.push_back((uint8_t)(color.x * 255.0f));
            samples.push_back((uint8_t)(color.y * 255.0f));
            samples.push_back((uint8_t)(color.z * 255.0f));
            samples.push_back((uint8_t)(color.w * 255.0f));
    ...
    void update_size_over_time_texture()
    {
        ...
        for (uint32_t i = 0; i < GRADIENT_SAMPLES; i++)
        {
            float size = m_start_size + size_diff /* ImGui::BezierValue(x, m_size_curve) * size_diff*/;
    ...
    void update_camera()
    {
        ...
        float forward_delta = m_heading_speed  * (float)m_delta;
        float right_delta   = m_sideways_speed * (float)m_delta;

        current->set_translation_delta(current->m_forward, forward_delta);
        current->set_translation_delta(current->m_right, right_delta);

        m_camera_x = (float)m_mouse_delta_x * m_camera_sensitivity;
        m_camera_y = (float)m_mouse_delta_y * m_camera_sensitivity;
...
private:
    ...
    int32_t       m_emission_rate          = 250;  // Particles per second
    ...
    glm::mat4     m_position_transform     = glm::mat4(1.0f);
